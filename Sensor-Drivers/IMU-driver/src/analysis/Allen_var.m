...
figure
loglog(tau, avar)
title('Allan Variance of Angular Velocity in z direction')
xlabel('\tau (sec)');
ylabel('\sigma(\tau^2)')
legend('$\sigma (rad/s)^2$', 'Interpreter', 'latex')
grid on
axis equal
...
xlabel('\tau (sec)')
ylabel('\sigma(\tau)')
legend('$\sigma (rad/sec)$', '$\sigma_N ((rad/sec)/\sqrt{Hz})$', ...
    '$\sigma_K ((rad/sec)\sqrt{Hz})$', '$\sigma_B (rad/sec)$', 'Interpreter', 'latex')
